...
#### Gate 3 文本收益预测基线与在线遥测（2026-09-07）

先在 155 条开发集运行 `gate3/text_benefit_baseline.py`，使用 13 个纯文本、调用前可获得的特征，
以“全量 Refiner 是否使字符错误数下降”为目标做确定性分层 5 折交叉验证。155 条中仅 17 条正向
收益；0.10 阈值会调用 82 条但精确率仅 17.073%、净错误减少 -12，0.15 阈值会调用 11 条但没有
正向收益。没有任一非空阈值同时满足精确率 ≥50% 与净错误减少 >0，因此文本特征收益预测器被
`reject`，不得接入在线路由。

检查当前 Qwen3-ASR vLLM streaming 实现后确认：公开状态仅有累计文本/语言及 chunk 生命周期，
没有 token 置信度、logprob 或 N-best。为获得不改变解码的在线特征，Gate1 worker 新增
`asr_stream_metrics`，每次累计假设更新记录 chunk 数、假设更新数、被替换前缀字符数、追加字符数
