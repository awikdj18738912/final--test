...
显式自我修正允许一个受限的大比例删除特例：只有路由原因包含
`explicit_self_correction`，且 Refiner 输出是源窗口中原本存在的非空连续片段时，才豁免默认
60% 修改比例限制。例如“我是一个苹果，嗯，不对，我是一个梨。”可修订为“我是一个梨。”；
包含新实质字符的输出仍由比例限制和内容一致性检查拒绝。前端事件时间线会显示具体拒绝原因。
